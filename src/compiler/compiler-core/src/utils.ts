...
/**
 * Returns an user-friendly displayable value for a prefab instance.
 */
export const getPrettyValue = (type: DiezType, serializedInstance: any): string => {
  ...
  switch (type.toString()) {
    ...
    case 'DropShadow': {
      const color = getPrettyValue('Color', serializedInstance.color);
      const offset = getPrettyValue('Point2D', serializedInstance.offset);
      return `color: ${color}, offset: ${offset}, radius: ${serializedInstance.radius}`;
    }
    case 'File': {
      return `file: ${serializedInstance.src}`;
    }
    case 'Fill': {
      const color = getPrettyValue('Color', serializedInstance.color);
      return `color: ${color}`;
    ...
    case 'LinearGradient': {
      const color = getPrettyValue('Color', serializedInstance.color);
      const start = getPrettyValue('Point2D', serializedInstance.start);
      const end = getPrettyValue('Point2D', serializedInstance.end);
      return `color: ${color}, start: ${start}, end: ${end}`;
    }
    case 'Panel': {
